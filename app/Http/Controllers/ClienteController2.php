...
class ClienteController extends Controller
{
    ...
    public function enviar(Request $request)
    {
        ...
        $itens = "";
        $texto = "🛍 *NOVO PEDIDO* - Danyllo Retalhos%0a".$cupom."%0a%0a👤 *Cliente:* ".$nome."%0a%f0%9f%93%8d _".$endereco. "_%0a".$telefone."0a%0a📦 *Produtos*%0a------------------------%0a";
        foreach ( $_SESSION['carrinho'] as $item){
            $dado = ProdutoModel::findOrFail($item[0]);
            $texto = $texto . "%e2%80%a2%20" . $dado->codigosistema ."%0a". explode(" ", $dado->nome)[0]." ". explode(" ", $dado->nome)[1]  ."%0aQtd: ". $item[1] .' '.$dado->und_medida. "s%0aR$ ".$dado->preco ."%0a%0a";
            $itens = $itens .  $item[1] ."X (".$dado->codigosistema." - ".explode(" ", $dado->nome)[0]." ". explode(" ", $dado->nome)[1]." - ".$dado->preco.") ";
        }
        ...
        $pedido->save();
        return redirect(url('https://api.whatsapp.com/send?phone=5583996551689&text='. $texto));
    }
